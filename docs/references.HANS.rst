...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
事件循环的切换核心是CurrentLoopStorage::getOrCreate()方法，根据编译选项和运行时环境，为当前线程 获取或创建 一个事件循环协程（EventLoopCoroutine）实例，
并通过线程局部存储（TLS）确保每个线程有且仅有一个实例。

当创建新的事件循环实例时，回根据不同平台使用不同的事件循环
以下代码是内部实现以及解析：
.. code-block:: c++
    :caption: 示例:使用libev
...
关键点：
preferLibevFlag 是原子标志位，允许运行时强制使用 libev。
主线程默认使用 QtEventLoopCoroutine（集成 Qt 事件循环），非主线程使用 libev。
...
.. code-block:: c++
    ...
    if (preferLibevFlag->loadAcquire()) {
        ...
        if (QCoreApplication::instance() && QCoreApplication::thread() == QThread::currentThread()) {
            // 主线程：使用 Qt 事件循环
            eventLoop.reset(new QtEventLoopCoroutine());
            eventLoop->setObjectName("qt_eventloop_coroutine");
        } else {
...
类似 libev 分支，但针对 Windows 平台使用 WinEventLoopCoroutine。

.. code-block:: c++
    :caption: 示例:默认使用 Qt 事件循环
...
当未定义 libev 或 Windows 特定实现时，直接使用 Qt 的事件循环。
...
当你启动当前http服务，然后在浏览器输入:http://127.0.0.1:8443/hello/ 就会显示hello
